...
class HistoriaClinicaFormWindows extends StatefulWidget {
  final void Function(HistoriaClinica historia) onSave;
  const HistoriaClinicaFormWindows({super.key, required this.onSave});

  @override
  State<HistoriaClinicaFormWindows> createState() =>
      _HistoriaClinicaFormWindowsState();
}

class _HistoriaClinicaFormWindowsState
    extends State<HistoriaClinicaFormWindows> {
  ...
  List<Paciente> get _pacientesFiltrados {
    ...
    return _pacientes
        .where(
          (u) =>
              ('${u.nombres} ${u.apellidos}').toLowerCase().contains(q) ||
